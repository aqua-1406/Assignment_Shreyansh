...
def read_prompts_from_csv(Oreo_test):
    ...
    with open(Oreo_test, 'r', encoding='utf-8') as file:
        ...
        for row in list(csv_reader)[1:26]:
            prompt = row[3]  # Assuming prompts are in column D (adjust index if needed)
...
def index():
    if request.method == 'POST':
        # Get user input for business-related information
        ...
        # Generate and run prompts for each line
        chatgpt_responses = []
